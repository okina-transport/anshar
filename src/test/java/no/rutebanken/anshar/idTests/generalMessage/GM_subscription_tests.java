package no.rutebanken.anshar.idTests.generalMessage;
...
public class GM_subscription_tests extends SpringBootBaseTest {
    ...
    @Test
    public void GM_useOriginalId_trueSiri2_1() throws InterruptedException {
        ...
        resetIdProcessings();
        IMap<SiriObjectStorageKey, GeneralMessage> originalSaved = generalMessages.getGeneralMessages();
        HazelcastTestMap<GeneralMessage> testMap = new HazelcastTestMap<>();
        generalMessages.setGeneralMessages(testMap);

        GeneralMessage generalMessage1 = TestUtils.createGeneralMessageForLineAndStop(sitNumber1, OLD_LINE1_REF, OLD_STOP1_REF);

        OutboundSubscriptionSetup outboundSubscription = createOutboundSubscription(true);
        outboundSubscription.setOutboundIdMappingPolicy(OutboundIdMappingPolicy.ORIGINAL_ID);
        serverSubscriptionManager.addSubscription(outboundSubscription);

        List<GeneralMessage> generalMessagesToIngest = new ArrayList<>();
        ...
        TestUtils.verifyStringInResponse(mockServer, "<StopPointRef>" + NEW_STOP1_REF + "</StopPointRef>");
        TestUtils.verifyStringInResponse(mockServer, " version=\"2.1\"");
        generalMessages.setGeneralMessages(originalSaved);
    }

    @Test
    public void GM_useOriginalId_true() throws InterruptedException {
        ...
        //Faire les vérifications
        TestUtils.verifyStringInResponse(mockServer, "<StopPointRef>" + NEW_STOP1_REF + "</StopPointRef>");
        TestUtils.verifyStringInResponse(mockServer, " version=\"2.0\"");
        generalMessages.setGeneralMessages(originalSaved);
    }

    @Test
    public void GM_useOriginalId_true_IDFM2_4() throws InterruptedException {
    ...
    @Test
    public void GM_useOriginalId_false() throws InterruptedException {
        initStopPlaceMapper();
        ...
        resetIdProcessings();
        IMap<SiriObjectStorageKey, GeneralMessage> originalSaved = generalMessages.getGeneralMessages();
        HazelcastTestMap<GeneralMessage> testMap = new HazelcastTestMap<GeneralMessage>();
        generalMessages.setGeneralMessages(testMap);

        GeneralMessage generalMessage1 = TestUtils.createGeneralMessageForLineAndStop(sitNumber1, OLD_LINE1_REF, OLD_STOP1_REF);

        OutboundSubscriptionSetup outboundSubscription = createOutboundSubscription(false);
        outboundSubscription.setOutboundIdMappingPolicy(OutboundIdMappingPolicy.DEFAULT);
        serverSubscriptionManager.addSubscription(outboundSubscription);
