...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template
        ...
        <xsl:choose>
            ...
            <xsl:when
                    test="local-name()='NotifyEstimatedTimetable' or local-name()='NotifyVehicleMonitoring' or local-name()='NotifySituationExchange' or local-name()='NotifyStopMonitoring' or local-name()='NotifyFacilityMonitoring' or local-name()='NotifyGeneralMessage'">
                <xsl:element name="siril:Siri">
                    <xsl:attribute name="version">
                        <xsl:value-of select="'2.0'"/>
                    </xsl:attribute>
            ...
            <xsl:when test="local-name()='Subscribe'">
                <xsl:element name="Siri" namespace="http://www.siri.org.uk/siri">
                    <xsl:element name="SubscriptionRequest" namespace="http://www.siri.org.uk/siri">
                        ...
                        <xsl:for-each select="Request/siril:EstimatedTimetableSubscriptionRequest">
                        ...
                        <xsl:for-each select="Request/siril:SituationExchangeSubscriptionRequest">
            ...
            <xsl:otherwise>
                <xsl:element name="siril:Siri">
                    <xsl:attribute name="version">
                        <xsl:choose>
                            <xsl:when test="Answer/child::node()/@version">
                                <xsl:value-of select="Answer/child::node()/@version"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="'2.0'"/>
