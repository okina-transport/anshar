...
import org.rutebanken.hazelcasthelper.service.RutebankenHazelcastException;
...
//        getSerializerConfigs().forEach( cfg.getSerializationConfig()::addSerializerConfig);
        SubZero.useAsGlobalSerializer(new SerializerConfig());
    ...
    private HazelcastInstance initForLocalHazelCast() {
        ...
        log.info("This is turbo mode HZ config.");

        log.info("Launching anshar with hazelcast clusterName: " + clusterName);
...
//                .setAutoDetectionConfig(new AutoDetectionConfig().setEnabled(false))
        ...
        Config serializerCfg = new Config();
        SubZero.useAsGlobalSerializer(serializerCfg);
